...
export default function ProjectDetailPage() {
  ...
  return (
    <AppShell
      ...
      action={
        <Link href="/projects" className="rounded-xl border border-slate-700 bg-slate-900 px-4 py-2.5 text-sm font-bold text-slate-200 hover:bg-slate-800">
          ← Quay lại
        </Link>
      }
    >
      {loading && <p className="py-16 text-center text-slate-400">Đang tải...</p>}
      {error && <p className="rounded-xl border border-rose-500/30 bg-rose-500/10 p-4 text-sm text-rose-300">{error}</p>}
      ...
      {details && (
        <div className="space-y-7">
          <section className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <span
                  className={`rounded-full px-2.5 py-1 text-xs font-bold ${
                    details.isArchived ? "bg-slate-700 text-slate-300" : "bg-emerald-500/10 text-emerald-300"
                  }`}
                >
                  {details.isArchived ? "Đã lưu trữ" : "Đang hoạt động"}
                </span>
                <h2 className="mt-4 text-2xl font-black text-white">{details.name}</h2>
              ...
              <div className="text-right text-sm text-slate-400">
                <p>
                  Tạo bởi <span className="font-bold text-slate-200">{details.createdByName || "Không rõ"}</span>
                </p>
          ...
          <div className="grid gap-6 xl:grid-cols-3">
            <section className="rounded-2xl bg-gradient-to-br from-sky-500 to-indigo-600 p-6 text-white xl:col-span-1">
              ...
              <div className="mt-6 h-2 rounded-full bg-indigo-300/50">
            ...
            <section className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6 xl:col-span-2">
              <h3 className="font-bold text-white">Thống kê công việc</h3>
              <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <StatCard label="Tổng công việc" value={summary?.totalTasks ?? 0} />
                <StatCard label="Đang làm" value={summary?.inProgressTasks ?? 0} accent="text-amber-300" />
                <StatCard label="Hoàn thành" value={summary?.completedTasks ?? 0} accent="text-emerald-300" />
                <StatCard label="Quá hạn" value={summary?.overdueTasks ?? 0} accent="text-rose-300" />
              </div>
          ...
          <div className="grid gap-6 xl:grid-cols-2">
            <section className="rounded-2xl border border-slate-800 bg-slate-900/80">
              <div className="border-b border-slate-800 p-5">
                <h3 className="font-bold text-white">Thành viên tham gia</h3>
                <p className="mt-1 text-xs text-slate-400">{details.members.length} người</p>
              </div>
                ...
                {details.members.map((member) => (
                  <div key={member.userId} className="flex items-center justify-between gap-4 p-5">
                    ...
                    <div className="text-right">
                      <span className="rounded-lg bg-slate-800 px-2 py-1 text-xs font-bold text-slate-200">
                        {projectRoleLabel[member.projectRole] || member.projectRole}
                      </span>
                      <p className="mt-2 text-xs text-slate-500">{member.assignedTaskCount} công việc được giao</p>
                    </div>
                  </div>
                ))}
                {!details.members.length && (
                  <p className="p-8 text-center text-sm text-slate-400">Chưa có thành viên nào.</p>
                )}
              </div>
            </section>
            ...
            <section className="rounded-2xl border border-slate-800 bg-slate-900/80">
              <div className="flex items-center justify-between border-b border-slate-800 p-5">
                <div>
                  <h3 className="font-bold text-white">Công việc gần đây</h3>
                  <p className="mt-1 text-xs text-slate-400">Cập nhật mới nhất trong dự án</p>
                </div>
                <Link href="/tasks" className="text-xs font-bold text-sky-400">
                  Mở bảng Kanban →
                </Link>
              </div>
              <div className="divide-y divide-slate-800">
                {details.recentTasks.map((task) => (
                  <div key={task.id} className="flex items-center justify-between gap-4 p-5">
                    ...
                    <div className="text-right">
                      <span className="text-xs font-bold text-slate-300">{statusLabel[task.status] || task.status}</span>
                ...
                {!details.recentTasks.length && (
                  <p className="p-8 text-center text-sm text-slate-400">Chưa có công việc nào trong dự án.</p>
                )}
              </div>
            </section>
          </div>
