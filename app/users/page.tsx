...
export default function UsersPage() {
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      if (form.id) {
        await api.put(`/users/${form.id}`, { id: form.id, fullName: form.fullName, systemRole: form.systemRole });
      } else {
        await api.post("/users", { fullName: form.fullName, email: form.email, password: form.password });
      }
      setForm(null);
      load();
    } catch {
  ...
  return (
    <AppShell
      ...
      action={
        <button
          onClick={() => setForm({ fullName: "", email: "", password: "", systemRole: "User" })}
    ...
    >
            ...
            {users.map((member) => {
              ...
              return (
                <tr key={member.id}>
                  ...
                  <td className="p-4 font-bold text-white">{item?.activeTasks ?? 0}</td>
                  <td className="p-4 font-bold text-emerald-300">{item?.completedTasks ?? 0}</td>
                  <td className="p-4 font-bold text-rose-300">{item?.overdueTasks ?? 0}</td>
                  <td className="p-4 whitespace-nowrap">
                    <button
                      onClick={() => setForm({ id: member.id, fullName: member.fullName, email: member.email, password: "", systemRole: member.systemRole })}
                      className="mr-3 text-xs font-bold text-sky-400"
                    >
                      Sửa
                    </button>
                    <button onClick={() => remove(member.id)} className="text-xs font-bold text-rose-400">
                      Xóa
                    </button>
                  </td>
                </tr>
      ...
      {form && (
        <div className="fixed inset-0 z-30 grid place-items-center bg-slate-950/70 p-4">
          <form onSubmit={save} className="w-full max-w-md space-y-4 rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-xl">
            <div className="flex justify-between">
              <h2 className="font-bold text-white">{form.id ? "Cập nhật thành viên" : "Thêm thành viên"}</h2>
            ...
            <select
              ...
              onChange={(event) => setForm({ ...form, systemRole: event.target.value as "Admin" | "User" })}
